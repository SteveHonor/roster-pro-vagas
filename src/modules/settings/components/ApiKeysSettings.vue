    ...
    <template v-if="isAdminOwner">
      ...
      <div v-if="!canUseApi" class="mx-auto max-w-2xl py-10">
        ...
        <div class="text-center">
          ...
          <h2 class="mt-4 text-2xl font-bold text-gray-900">Integre o Roster Pro com qualquer sistema</h2>
          <p class="mt-3 text-sm leading-relaxed text-gray-500">
            Conecte escalas, usuários e equipes ao seu ERP, BI ou ferramentas internas.<br>
            Automatize fluxos e elimine trabalho manual com a API REST do Roster Pro.
          </p>
        </div>
...
v-for="uc in useCases" :key="uc.label"
        ...
        <div class="mt-8 overflow-hidden rounded-xl bg-gray-900 shadow-lg">
          <div class="flex items-center gap-1.5 border-b border-gray-700 px-4 py-3">
            ...
            <span class="ml-3 text-xs text-gray-500">GET /api/v1/schedules</span>
          </div>
          <div class="p-5 font-mono text-xs leading-6">
            <div><span class="text-purple-400">curl</span> <span class="text-gray-300">https://api.rosterpro.com.br/api/v1/schedules</span> <span class="text-gray-500">\</span></div>
            ...
            <div class="pl-10"><span class="text-gray-400">{ "id": 1, "status": "</span><span class="text-green-400">confirmed</span><span class="text-gray-400">", "user": "Ana Lima", "team": "UTI" },</span></div>
            <div class="pl-10"><span class="text-gray-400">{ "id": 2, "status": "</span><span class="text-yellow-400">pending</span><span class="text-gray-400">", "user": "João Silva", "team": "PA" }</span></div>
...
<span class="text-gray-400">}</span></pre>
              ...
              <div class="flex items-start gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                ...
                <p class="text-xs text-gray-500">
                  ...
                  incluindo criação de escalas, gerenciamento de equipes e notificações.
        ...
        <div class="overflow-hidden rounded-xl border border-violet-200 bg-violet-50/30">
          ...
          >
            <div class="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p class="text-xs text-gray-500">Use linguagem natural para consultar escalas e gerenciar equipes.</p>
              </div>
            </div>
          ...
          <div v-if="showMcp" class="border-t border-violet-200 px-5 pb-5 pt-4 space-y-4">
            <p class="text-xs text-gray-600 leading-relaxed">
              ...
              Consulte escalas, confirme presenças e envie mensagens para a equipe por linguagem natural — sem abrir o app.
            </p>
        ...
        <span class="text-blue-400">"Authorization"</span>: <span class="text-orange-400">"Bearer SUA_API_KEY"</span>
            ...
            <div class="flex items-start gap-3 rounded-lg border border-violet-200 bg-violet-50 p-3">
              ...
              <p class="text-xs text-violet-700">
                Após configurar, reinicie o assistente. Pergunte: <em>"Quem não confirmou a escala essa semana?"</em>
              </p>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      revokeTarget: null,
      endpoints: [
        { method: 'GET',   path: 'users',                   description: 'Lista colaboradores da conta' },
        { method: 'GET',   path: 'teams',                   description: 'Lista equipes' },
        { method: 'GET',   path: 'teams/:id/members',       description: 'Membros e posições de uma equipe' },
        { method: 'GET',   path: 'events',                  description: 'Lista agendas/eventos' },
        { method: 'GET',   path: 'schedules',               description: 'Lista escalas (filtros: status, from, to, user_id)' },
        { method: 'PATCH', path: 'schedules/:id/respond',   description: 'Confirma ou recusa uma escala' },
        { method: 'GET',   path: 'dashboard',               description: 'Resumo geral da conta' },
        { method: 'POST',  path: 'users/:id/absences',      description: 'Cria afastamento para um colaborador' },
        { method: 'POST',  path: 'broadcasts',              description: 'Envia mensagem broadcast para a equipe' },
      ],
      useCases: [
        { label: 'Sincronize com ERP',         icon: 'Squares'      },
        { label: 'Exporte para BI',             icon: 'ChartBar'     },
        { label: 'Integre com sistema de RH',   icon: 'Users'        },
        { label: 'Automatize relatórios',       icon: 'DocumentText' },
        { label: 'Atualizações em tempo real',  icon: 'ArrowPath'    },
        { label: 'Workflows personalizados',    icon: 'Cog'          }
      ]
    };
  },
  computed: {
